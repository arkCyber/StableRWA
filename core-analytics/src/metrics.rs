...
pub trait MetricsCollector: Send + Sync {
    ...
    async fn register_metric_definition(&self, definition: MetricDefinition) -> AnalyticsResult<()>;
...
impl MetricsCollector for InMemoryMetricsCollector {
    async fn collect_metric(&self, metric: Metric) -> AnalyticsResult<()> {
        ...
        if metrics.len() > self.config.max_metrics_per_collection {
            metrics.drain(0..metrics.len() - self.config.max_metrics_per_collection);
        }
    ...
    async fn get_aggregated_metrics(
        ...
        for metric in &metrics {
            ...
            grouped_metrics.entry(interval_start).or_default().push(metric);
        }
        
        // Apply aggregation function
        let mut aggregated = Vec::new();
        for (timestamp, group_metrics) in grouped_metrics {
            if let Some(aggregated_value) = self.apply_aggregation_function(&group_metrics, function) {
                aggregated.push(AggregatedMetric {
                    ...
                    tags: group_metrics.first().map(|m| m.tags.clone()).unwrap_or_default(),
                });
            }
        }
    ...
    async fn delete_metrics(
        ...
        metrics.retain(|m| {
            !(m.name == name
                && time_range.as_ref().map_or(true, |tr| m.timestamp >= tr.start && m.timestamp <= tr.end)
                && tags.as_ref().map_or(true, |filter_tags| {
                    filter_tags.iter().all(|(k, v)| m.tags.get(k) == Some(v))
                }))
        });
    ...
    async fn get_metric_statistics(&self, name: &str) -> AnalyticsResult<MetricStatistics> {
        ...
        for metric in &filtered_metrics {
            unique_tags.insert(&metric.tags);
        }
...
impl InMemoryMetricsCollector {
    ...
    fn apply_aggregation_function(&self, metrics: &[&Metric], function: AggregationFunction) -> Option<MetricValue> {
        if metrics.is_empty() {
            return None;
        }
...
mod tests {
    ...
    async fn test_metrics_collector() {
        ...
        let retrieved_metrics = collector.get_metric("test_metric", time_range, None).await.unwrap();
