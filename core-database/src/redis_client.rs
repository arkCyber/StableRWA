...
impl RedisClient {
    ...
    pub async fn ping(&self) -> Result<(), DatabaseError> {
        let mut conn = self.client.get_async_connection().await.map_err(|e| {
            DatabaseError::Connection(format!("Failed to get Redis connection: {}", e))
        })?;
        redis::cmd("PING")
...
impl Cache for RedisClient {
        ...
        T: Serialize + Send + Sync,
    {
        ...
        match ttl {
            Some(seconds) => {
                let _: () = conn.set_ex(key, json_str, seconds).await.map_err(|e| {
                    DatabaseError::Connection(format!("Redis set_ex failed: {}", e))
                })?;
            }
            None => {
                let _: () = conn
...
mod tests {
    ...
    fn test_cache_keys() {
        ...
        assert_eq!(
            CacheKeys::rate_limit("192.168.1.1"),
            "rate_limit:192.168.1.1"
        );
        assert_eq!(
            CacheKeys::blockchain_cache("ethereum", "block_123"),
            "blockchain:ethereum:block_123"
        );
    }
