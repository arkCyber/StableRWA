...
use crate::{DidError, DidResolver, DidResult};
...
pub trait DidVerifier: Send + Sync {
    ...
    async fn verify_capability(&self, did: &str, capability: &str, key_id: &str)
        -> DidResult<bool>;
}
...
impl DidVerifier for StandardVerifier {
    async fn verify_signature(
        ...
        self.verify_with_method(verification_method, data, signature)
            .await
    }
    ...
    async fn verify_capability(
        ...
        match capability {
            "authentication" => Ok(document.authentication.iter().any(|auth| match auth {
            ...
            "assertionMethod" => {
                Ok(document
                    .assertion_method
                    .iter()
                    .any(|assertion| match assertion {
                        crate::VerificationMethodReference::Id(id) => id == key_id,
                        crate::VerificationMethodReference::Embedded(vm) => vm.id == key_id,
                    }))
            }
            "keyAgreement" => Ok(document
                .key_agreement
                .iter()
                .any(|agreement| match agreement {
                    crate::VerificationMethodReference::Id(id) => id == key_id,
                    crate::VerificationMethodReference::Embedded(vm) => vm.id == key_id,
                })),
            ...
            "capabilityDelegation" => {
                Ok(document
                    ...
                    .iter()
                    .any(|delegation| match delegation {
                        crate::VerificationMethodReference::Id(id) => id == key_id,
                        crate::VerificationMethodReference::Embedded(vm) => vm.id == key_id,
                    }))
            }
            _ => Err(DidError::InvalidVerificationMethod(format!(
                "Unknown capability: {}",
                capability
            ))),
        }
...
impl StandardVerifier {
    ...
    async fn verify_with_method(
        ...
        match verification_method.method_type.as_str() {
            "Ed25519VerificationKey2020" => {
                self.verify_ed25519(verification_method, data, signature)
                    .await
            }
            "EcdsaSecp256k1VerificationKey2019" => {
                self.verify_secp256k1(verification_method, data, signature)
                    .await
            }
            _ => Err(DidError::InvalidVerificationMethod(format!(
    ...
    async fn verify_ed25519(
        ...
        let public_key_bytes = match &verification_method.public_key {
            crate::PublicKeyMaterial::Base58 { public_key_base58 } => BASE64
                .decode(public_key_base58)
                .map_err(|e| DidError::InvalidKeyFormat(e.to_string()))?,
            crate::PublicKeyMaterial::Multibase {
                public_key_multibase,
            } => {
                // Remove multibase prefix and decode
                if public_key_multibase.starts_with('z') {
                    BASE64
                        .decode(&public_key_multibase[1..])
                        .map_err(|e| DidError::InvalidKeyFormat(e.to_string()))?
                } else {
        ...
        let verifying_key =
            VerifyingKey::from_bytes(&public_key_bytes.try_into().map_err(|_| {
                DidError::CryptographicError("Invalid public key length".to_string())
            })?)
...
mod tests {
    use super::*;
    use crate::{
        Did, DidDocument, KeyManager, KeyPurpose, KeyType, MemoryResolver,
    ...
    async fn test_signature_verification() {
        ...
        let is_valid = verifier
            .verify_signature(&did.to_string(), "did:rwa:123#key-1", data, &signature)
            .await
            .unwrap();
    ...
    async fn test_capability_verification() {
        ...
        let has_capability = verifier
            .verify_capability(&did.to_string(), "authentication", "did:rwa:123#key-1")
            .await
            .unwrap();
        ...
        let no_capability = verifier
            .verify_capability(&did.to_string(), "keyAgreement", "did:rwa:123#key-1")
            .await
            .unwrap();
