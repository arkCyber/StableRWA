...
use crate::{ContentMetadata, IpfsConfig, IpfsError, IpfsHash, IpfsResult};
...
impl IpfsClientTrait for IpfsClient {
    ...
    async fn get_bytes(&self, hash: &IpfsHash) -> IpfsResult<Vec<u8>> {
        ...
        info!(
            "Successfully retrieved {} bytes from IPFS: {}",
            response.len(),
            hash
        );
        Ok(response)
    ...
    async fn size(&self, hash: &IpfsHash) -> IpfsResult<u64> {
        ...
        let stat =
            self.client.object_stat(hash.as_str()).await.map_err(|e| {
                IpfsError::ClientError(format!("Failed to get content stats: {}", e))
            })?;
    ...
    async fn list_pins(&self) -> IpfsResult<Vec<IpfsHash>> {
        ...
        let hashes: Result<Vec<_>, _> = pins
            .keys
            .into_iter()
    ...
    async fn get_file<P: AsRef<Path> + Send>(&self, hash: &IpfsHash, path: P) -> IpfsResult<()> {
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                IpfsError::ClientError(format!("Failed to create directories: {}", e))
            })?;
        }
        ...
        info!(
            "Successfully saved file from IPFS: {} -> {}",
            hash,
            path.display()
        );
        Ok(())
...
mod tests {
    ...
    async fn test_mock_client_exists_and_size() {
        ...
        let fake_hash =
            IpfsHash::new("QmFakeHashThatDoesNotExist123456789012345678".to_string()).unwrap();
