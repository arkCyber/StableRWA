...
use crate::{
    IpfsError, IpfsResult, IpfsHash, IpfsConfig, ContentMetadata, StorageStats,
    client::{IpfsClientTrait, IpfsClientFileOps, IpfsClient}
};
...
pub trait StorageManagerTrait {
    ...
    async fn store_content(&self, data: Vec<u8>, metadata: ContentMetadata) -> IpfsResult<StorageResult>;
    
    /// Store file with automatic metadata generation
    async fn store_file<P: AsRef<Path> + Send>(&self, path: P, name: Option<String>) -> IpfsResult<StorageResult>;
...
impl StorageManager {
    ...
    fn validate_content(&self, metadata: &ContentMetadata) -> IpfsResult<()> {
        ...
        if !self.config.allowed_mime_types.is_empty() 
            ...
            return Err(IpfsError::ValidationError(format!(
                "MIME type {} is not allowed", metadata.mime_type
            )));
        }
...
impl StorageManagerTrait for StorageManager {
    ...
    async fn store_content(&self, data: Vec<u8>, mut metadata: ContentMetadata) -> IpfsResult<StorageResult> {
        debug!("Storing content: {} bytes, name: {}", data.len(), metadata.name);
        ...
        // Store metadata
        self.metadata_store.write().await.insert(hash.as_str().to_string(), metadata.clone());
    ...
    async fn store_file<P: AsRef<Path> + Send>(&self, path: P, name: Option<String>) -> IpfsResult<StorageResult> {
        ...
        // Generate metadata
        let metadata = self.generate_file_metadata(path, name, hash.clone()).await?;
        ...
        info!("Successfully stored file: {} -> {}", path.display(), result.hash);
        Ok(result)
    }
    ...
    async fn get_metadata(&self, hash: &IpfsHash) -> IpfsResult<ContentMetadata> {
        ...
        self.metadata_store.read().await
            .get(hash.as_str())
            .cloned()
            .ok_or_else(|| IpfsError::MetadataError(format!("Metadata not found for hash: {}", hash)))
    }
    ...
    async fn update_metadata(&self, hash: &IpfsHash, metadata: ContentMetadata) -> IpfsResult<()> {
        ...
        if !self.client.exists(hash).await? {
            return Err(IpfsError::FileNotFound(format!("Content not found: {}", hash)));
        }
        
        // Update metadata store
        self.metadata_store.write().await.insert(hash.as_str().to_string(), metadata);
    ...
    async fn get_stats(&self) -> IpfsResult<StorageStats> {
        ...
        info!("Storage stats: {} files, {} bytes total", stats.total_files, stats.total_size);
        Ok(stats)
    }
    ...
    async fn search_by_mime_type(&self, mime_type: &str) -> IpfsResult<Vec<ContentMetadata>> {
        ...
        info!("Found {} items with MIME type: {}", results.len(), mime_type);
        Ok(results)
    }
}
...
mod tests {
    ...
    async fn test_store_and_retrieve_content() {
        ...
        // Store content
        let result = manager.store_content(data.clone(), metadata.clone()).await.unwrap();
    ...
    async fn test_store_file() {
        ...
        // Store file
        let result = manager.store_file(temp_file.path(), Some("my_test_file.txt".to_string())).await.unwrap();
    ...
    async fn test_metadata_operations() {
        ...
        let hash = IpfsHash::new("QmMetadataTest123456789012345678901234567890".to_string()).unwrap();
        ...
        manager.update_metadata(&result.hash, metadata.clone()).await.unwrap();
    ...
    async fn test_list_and_search_content() {
        ...
        manager.store_content(b"image1".to_vec(), metadata1).await.unwrap();
        manager.store_content(b"document".to_vec(), metadata2).await.unwrap();
        manager.store_content(b"image2".to_vec(), metadata3).await.unwrap();
        ...
        // Search by tags
        let photo_content = manager.search_by_tags(&["photo".to_string()]).await.unwrap();
        ...
        let pdfs = manager.search_by_mime_type("application/pdf").await.unwrap();
    ...
    async fn test_storage_stats() {
        ...
        manager.store_content(b"content1".to_vec(), metadata1).await.unwrap();
        manager.store_content(b"content2".to_vec(), metadata2).await.unwrap();
    ...
    async fn test_delete_content() {
        ...
        let hash = IpfsHash::new("QmDelete123456789012345678901234567890123456".to_string()).unwrap();
        let metadata = ContentMetadata::new(
            hash.clone(),
    ...
    fn test_storage_result() {
        let hash = IpfsHash::new("QmResult123456789012345678901234567890123456".to_string()).unwrap();
        let metadata = ContentMetadata::new(
            hash.clone(),
