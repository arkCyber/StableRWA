...
pub trait RateLimiter: Send + Sync {
    ...
    async fn is_allowed(&self, key: &str, limit: u32, window_seconds: u64) -> Result<bool, SecurityError>;
...
impl RateLimiter for InMemoryRateLimiter {
    async fn is_allowed(&self, key: &str, limit: u32, window_seconds: u64) -> Result<bool, SecurityError> {
        ...
        let entry = storage.entry(key.to_string()).or_insert_with(|| {
            RateLimitInfo {
                key: key.to_string(),
                requests: 0,
                limit,
        ...
        entry.requests += 1;
        debug!("Rate limit check for key: {} - {}/{}", key, entry.requests, entry.limit);
    ...
    async fn cleanup(&self) -> Result<(), SecurityError> {
        ...
        storage.retain(|_, info| {
            ...
            !info.is_expired() || (now - info.window_start).num_seconds() < 3600 // Keep for 1 hour
        });
...
impl Default for RateLimitConfig {
    fn default() -> Self {
        ...
        rules.insert("/auth/login".to_string(), RateLimitRule {
        ...
        rules.insert("/api/v1/assets".to_string(), RateLimitRule {
        ...
        rules.insert("/api/v1/payments".to_string(), RateLimitRule {
...
impl RateLimitMiddleware {
    ...
    pub async fn check_rate_limit(
        ...
        let usage = self.limiter.get_usage(key).await?.unwrap_or_else(|| {
            RateLimitInfo {
                key: key.to_string(),
                requests: 0,
                limit,
...
impl RateLimitResult {
    ...
    pub fn to_headers(&self) -> Vec<(String, String)> {
        vec![
            ("X-RateLimit-Limit".to_string(), self.usage.limit.to_string()),
            ("X-RateLimit-Remaining".to_string(), self.usage.remaining().to_string()),
            ("X-RateLimit-Reset".to_string(), self.usage.reset_time.timestamp().to_string()),
            ("X-RateLimit-Window".to_string(), format!("{:?}", self.window)),
        ]
    }
}
...
mod tests {
    ...
    fn test_rate_limit_key_generator() {
        assert_eq!(RateLimitKeyGenerator::by_ip("192.168.1.1"), "ip:192.168.1.1");
