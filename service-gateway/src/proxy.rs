...
impl ProxyClient {
    ...
    pub async fn proxy_request(
        ...
        let response_body = response.bytes().await.map_err(|e| {
            GatewayError::ProxyError(format!("Failed to read response body: {}", e))
        })?;

        let duration = start_time.elapsed();
        info!(
            "Proxy request completed in {:?} with status {}",
            duration, status
        );
...
pub async fn proxy_to_asset_service(
    ...
    match proxy_client
        .proxy_request(&target_url, method, req.headers(), body_option)
        .await
    {
